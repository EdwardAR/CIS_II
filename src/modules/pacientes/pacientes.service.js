  ...
  return db
    .prepare(
      `SELECT u.id as user_id, u.full_name, u.email, p.id as patient_id, p.dni, p.phone,
              p.birth_date, p.address, p.emergency_name, p.emergency_phone, p.emergency_relation
       FROM users u
...
function updatePatientProfile(userId, payload) {
  const tx = db.transaction(() => {
    ...
    db.prepare('UPDATE patients SET phone = ?, address = ?, emergency_name = ?, emergency_phone = ?, emergency_relation = ? WHERE user_id = ?').run(
      payload.phone,
      payload.address,
